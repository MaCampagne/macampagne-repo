...
<?php while ( $bne_flyout->have_posts() ) : $bne_flyout->the_post();
	...
	$flyout_size = get_field( 'bne_flyout_size' );
	...
	if( !$flyout_size && get_field( 'bne_flyout_container_width' ) ) {
		$flyout_size = get_field( 'bne_flyout_container_width' );
	}
	...
	if( 'Left Side' == get_field( 'bne_flyout_location' ) ) {
		...
		$flyout_size = 'height:'.$flyout_size.'px; top:-'.$flyout_size.'px;';
	} elseif( 'Bottom Side' == get_field( 'bne_flyout_location' ) ) {
		$flyout_size = 'height:'.$flyout_size.'px; bottom:-'.$flyout_size.'px;';
	}

	// Flyout Styles
	$flyout_bg_color 		= get_field( 'bne_flyout_background_color' );
	$flyout_bg_image 		= get_field( 'bne_flyout_background_image' );
	$flyout_bg_repeat 		= get_field( 'bne_flyout_background_image_repeat' );
	$flyout_bg_position 	= get_field( 'bne_flyout_background_image_position' );
	$flyout_bg_attachment 	= get_field( 'bne_flyout_background_image_attachement' );
	$body_color 			= get_field( 'bne_flyout_content_font_color' );
	$link_color 			= get_field( 'bne_flyout_content_font_link_color' );
	$header_color 			= get_field( 'bne_flyout_content_font_header_color' );
